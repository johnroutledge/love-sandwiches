...
def get_sales_data():
    """
    Get sales data inputs from user
    Run a while loop to collect a valid string of data from the user
    via the terminal , which must be a string a six numbers seperated
    by a comma. The loop will repeatedly request data from the user
    until valid.
    """
    while True:
        print("Please enter sales figures from last market")
...
def validate_data(values):
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                f"Exactly 6 values required, but you entered {len(values)}"
            )
...
def calculate_surplus_data(sales_row):
    ...
    """
    print("Calulcating surplus data...\n")
...
def update_worksheet(worksheet, data):
    """
    Update relevant worksheet, add new row with list that data provided
    """
...
def calculate_stock_data(data):
    """
    Calculate the average stock for each sandwich type and add 10%
    """
...
def get_stock_values(data):
    """
    Gets sandwich headings from worksheet, stock data for next market 
    ...
    for i in range(len(headings)):
        make_sandwiches[headings[i]]=data[i]
